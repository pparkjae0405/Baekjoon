...
05. 1157
```
입력된 문자열 중 가장 많이 사용된 알파벳을 대문자로 출력하는 문제
입력은 대/소문자, 여러개일 경우 ? 출력
1. 문자열 a를 입력받고 소문자로 변환, 최다 알파벳 위치 where, 
   알파벳값 value, 복수 최댓값 인식 Manymax 선언
...
5. 5. 최댓값이 여러개가 아니라면 해당 알파벳을 대문자로 출력
```
코드
```
import java.io.*;
