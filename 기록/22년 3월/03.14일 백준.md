...
10. 1316
```
중복되지 않은 그룹 단어의 개수를 세는 문제

1. 그룹 단어 카운트 count 선언, 반복 횟수 a 입력
2. 그룹 단어 체크하고 참이면 count 증가하여 출력
3. 그룹단어 확인 check 함수 선언
4. 알파벳 배열 선언, 그 전 단어의 아스키 값 저장 prev 선언
5. 단어를 입력받고, 그룹단어인지 체크하여 참/거짓 리턴
```
코드
```
import java.io.*;
